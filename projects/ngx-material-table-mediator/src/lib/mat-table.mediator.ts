...
import {FetchPayload, MediatorData} from './models';
 ...
 * It takes care of:
 * - firing the `fetch` function when the `fetchPayload$`, sorting or page changes
 * - resetting page index to 0, if `fetchPayload$` or sorting changes
 * - error handling (retries or emits error$ and returns empty data array)
 * - indicating loading
 * - completing all observables, if ngOnDestroy() is called
 *
 * After creating an object of the mediator, it will automatically be ready to load the data, as soon as a fetchPayload$ is emitted.
 *
 * You can change the behaviour by overwriting the init functions (For example not calling #initDataFetch in #ngOnInit).
 *
 * **Required Generics**
 * - #1 <F> &rarr; type of the fetch payload to be used in the underlying `fetch` function, for example a form output
...
export abstract class MatTableMediator<F, O> implements OnInit, OnDestroy {
     ...
     * @example
     * protected fetchPayload$ = of(undefined);
     */
    protected abstract get fetchPayload$(): FetchPayload<F>;
     ...
     */
    constructor(protected readonly table: MatTable<O>,
     ...
     * @param payload the latest fetch payload
    ...
    /**
     * This function creates an internal observable to reset the paginator, if sorting or fetch payload changes.
     */
    protected initPageReset() {
        merge(this.sortChange$, this.fetchPayload$).pipe(
